...
边界有效性仍只是协议门槛，并不能证明全跨度文本的忠实性。公开指标还会报告：

- `full_length_ratio`：抽取输出 token 数除以采集时的计费输出 token 数；
- `full_length_alignment`：对截断与过度扩写都施加惩罚的对数对称分数；
- `summary_expansion_ratio`：在同一个冻结词法 tokenizer 下，还原文本与供应商摘要的长度比；
- `summary_ngram_containment`：检测摘要连续片段是否被原样嵌入，避免无关填充掩盖复制。

指标校准探针还会使用严格有序、可精确匹配的内容锚点。这些诊断项目前**不会**修改生产环境的
`strong_recovery` 阈值；只有在合成对照与供应商支持的对照实验上冻结阈值后，才能升级门槛。
Token 长度只是代理信号，不是越长越好的优化目标；上述指标也都不能证明供应商原文被逐字还原。
...
7. 排名首先依据场景宏平均 `strong_recovery` 率，其次才是稳健质量、有效性、抗摘要复制能力、
   拒答 / 泄漏率与波动性。**不要**用抽取提示去优化答案正确性、原始长度或金丝雀饱和度。
